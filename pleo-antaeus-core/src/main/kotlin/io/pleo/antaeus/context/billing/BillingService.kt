...
class BillingService(
    ...
    suspend fun execute(command: CloseBillingInvoiceCommand): Either<Throwable, Event> = Either.catch {
        ...
        billing.closeInvoice(command.invoicesId).let { billing ->
            when (billing.isComplete()) {
                true -> BillingCompletedEvent(billing.processId)
                else -> BillingInvoiceCompletedEvent(billing.processId, command.invoicesId)
            }
        }
    }
