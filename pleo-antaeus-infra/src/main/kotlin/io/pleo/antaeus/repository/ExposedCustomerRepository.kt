...
class ExposedCustomerRepository(private val db: Database) : CustomerRepository {
    override fun createCustomer(currency: Currency): Customer = transaction(db) {
        return@transaction CustomerEntity.new {
            this.currency = currency.toString()
        }.toCustomer()
    }
    
    override fun fetchAll(): List<Customer> = transaction(db) {
        CustomerEntity.all().map { it.toCustomer() }
    }

    override fun load(id: Int): Customer? = transaction(db) {
